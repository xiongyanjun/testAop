...
public class SmartEditor {
    ...
    private static String[] handle(String[] str) {
        for (int i = 0; i < str.length; i++) {
            str[i] = proofreading(str[i]);
        }
        return str;
    }

    private static String proofreading(String str) {
        for (int i = 0; i < str.length(); i++) {
            char[] chars = str.toCharArray();
            if (i < chars.length - 1 && chars[i] == chars[i+1]){
                if (chars[i] == chars[i+2]){
                    str = new StringBuilder(str).deleteCharAt(i+2).toString();
                    i = 0;
                }
                else if (i < chars.length - 2 && i < chars.length - 3 && chars[i+2] == chars[i+3]){
                    str = new StringBuilder(str).deleteCharAt(i+2).toString();
                    i = 0;
                }
            }
